...
public class TestGenerator {
    ...
    public String generateJUnitTestForGet(List<QueryInfo> selectQueries, String relativeHttpUrl,
            Application application, String contentType, String content, TestGenerationConfig testGenerationConfig,
            QuickSqlTestData quickSqlTestData, JUnitVersion jUnitVersion) {

        String fileNameWithoutExtension = ResourceFileNameGenerator.INSTANCE
                .buildFileNameWithoutExtension(relativeHttpUrl);
        ...
        if (!selectQueries.isEmpty()) {
            TestFile sqlFile = SqlScriptFileGenerator.INSTANCE
                    .generate(selectQueries, quickSqlTestData, fileNameWithoutExtension);
        ...
        TestFile javaTestFile = JavaTestClassGenerator.INSTANCE.generateJunitTestClass(
                relativeHttpUrl, application, optionalSqlFile, expectedResponseFile.name, httpContentType,
                jUnitVersion);

        String javaDir = testGenerationConfig.getJavaClassFolder();
        javaTestFile.writeInto(javaDir);

        return buildTestGenerationReport(optionalSqlFile, expectedResponseFile, javaTestFile, jUnitVersion);
